...
def decodeProduction(nodelist: "list[Customer]",sequence:list):
    startTime = 0
    for seq in sequence:
...
def decode(nodelist:"list[Customer]", route:"list[int]", allocation:"dict[[int],list[int]]",sequence:"list[int]",
           vehdistdict:"dict[tuple[int,int], int]", cptohddistdict:"dict[tuple[int,int,int]]"):
    decodeProduction(nodelist, sequence) # setting production time
    rt = deepcopy(route)
    del  rt[0], rt[len(rt) - 1]
    startTime = 0
    departureTime = 0
...
def Gantt(nodelist:"list[Customer]", veharclist:"list[tuple[int,int]]",cptohdarclist:"list[tuple[int,int]]",sequence:"list[int]",
          vehdistdict:"dict[tuple[int,int],int]", cptohddistdict:"dict[tuple[int,int], int]", customernum, homecustomernum, seed):
    startTime = 0
    dict1 = {}
    for item in veharclist:
        if item[0] == 0:
    ...
    for key,value in dict1.items():
        ...
        ax.text(value[0]+value[1]+0.5, key, f"{value[0] + value[1]}", va='center')
    ...
    plt.tight_layout()
    plt.savefig(f"image/Gantt_{customernum}_{homecustomernum}_{seed}.png")
    # plt.show()



def ShowRoute(nodelist:"list[Customer]", vehArcList:"list", cptohdArcList: "list", customernum, homecustomernum, seed):
    xcoords = []
    ycoords = []
    coordslist = []
    ...
    # plt.show()
    plt.savefig(f"image/opt_{customernum}_{homecustomernum}_{seed}.png")
